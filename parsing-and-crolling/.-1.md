...
url="https://cafe.naver.com/ArticleRead.nhn?clubid=10050146&page=1&menuid=749&inCafeSearch=true&searchBy=0&query=%BE%C6%C0%CC%C6%D0%B5%E5&includeAll=&exclude=&include=&exact=&searchdate=all&media=0&sortBy=date&articleid=653469961&referrerAllArticles=false"
# url="https://cafe.naver.com/ArticleRead.nhn?clubid=10050146&page=1&inCafeSearch=true&searchBy=1&query=%BE%C6%C0%CC%C6%D0%B5%E5&includeAll=&exclude=&include=&exact=&searchdate=all&media=0&sortBy=date&articleid=653359281&referrerAllArticles=true"
...
child = tt.findChildren()
# child = tt.find_all(['p', 'span','div',])
index = None
for i in range(len(child)):
    try:
        class_ = child[i]['class']
        if class_[0] in 'notice_manager':
            index = i
            break
    except:
        continue
...
for node in child:
    ...
    # list.append(node.get_text("",strip=True))
...
# print(len(tt.text),type(tt.text))
